...
def parse_args() -> argparse.Namespace:
    ...
    implementation_group = parser.add_mutually_exclusive_group()
    implementation_group.add_argument(
        "--no-numba",
        action="store_true",
        help="Skip the Numba baseline rollout and state comparison.",
    )
    implementation_group.add_argument(
        "--numba-only",
        action="store_true",
        help="Run only the Numba rollout benchmark.",
    )
...
def main() -> None:
    ...
    device = None if args.numba_only else select_device(args.backend)
    if device is not None:
        validate_dtype_for_device(dtype, device)
    backend_label = (
        None
        if device is None
        else "jax-cpu" if device.platform == "cpu" else f"jax-{device.platform}"
    )
    ...
    numba_instantiate_elapsed = None
    cpu_elapsed = None
    cpu_state = None
    if not args.no_numba:
        ...
        ), "Numba rollout did not end at final_time."
        cpu_state = collect_cpu_state(cpu_rods)

    jax_instantiate_elapsed = None
    jax_elapsed = None
    jax_state = None
    if not args.numba_only:
        assert device is not None, "JAX device must be selected when JAX is enabled."
    ...
    report_lines = [
        ...
        f"numba_only: {args.numba_only}",
        f"no_external_loads: {args.no_external_loads}",
        f"transfer_guard: {args.transfer_guard}",
    ]
    if not args.no_numba:
        assert (
            numba_instantiate_elapsed is not None
        ), "Numba instantiation timing must be available when Numba is enabled."
        assert cpu_elapsed is not None, (
            "Numba rollout timing must be available when Numba is enabled."
        )
        assert cpu_state is not None, (
            "Numba state must be available when Numba is enabled."
        )
        report_lines.extend(
            (
    ...
    if not args.numba_only:
        assert backend_label is not None, (
            "JAX backend label must be available when JAX is enabled."
        )
        assert jax_instantiate_elapsed is not None, (
            "JAX instantiation timing must be available when JAX is enabled."
        )
        assert jax_elapsed is not None, (
            "JAX rollout timing must be available when JAX is enabled."
        )
        report_lines.extend(
            (
                f"device: {device}",
    ...
    if not args.no_numba and not args.numba_only:
        assert cpu_elapsed is not None, (
            "Numba rollout timing must be available for the speedup comparison."
        )
        assert jax_elapsed is not None, (
            "JAX rollout timing must be available for the speedup comparison."
        )
        assert jax_state is not None, (
            "JAX state must be available for the Numba comparison."
        )
        assert cpu_state is not None, (
            "Numba state must be available for the JAX comparison."
        )
        report_lines.extend(
            (
